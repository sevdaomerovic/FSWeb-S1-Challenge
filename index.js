...
console.log("Görev 2: ", fenomenler[6]);
...
function indekseGoreFenomen(dizi, index) {
  const fenomeniSec = dizi[index];
  return `${index}. indekste bulunan fenomen: ${fenomeniSec.profile}`;
}
...
function fenomenEkle(fenomenler, number, profile, followers, posts, platform) {
  ...
  newArray.push(newFenomen);
  return newArray;
}

const newArray = fenomenEkle(
  fenomenler,
  6,
  "Workintech",
  10000000,
...
function enFenomenler(array) {
  ...
  for (let key in array) {
    if (array[key]["followers"] > 100000000) {
      endArray.push(array[key].profile);
    }
  }
  return endArray;
}
...
function fenomenGonderimSayisi(dizi, profil) {
  for (let i = 0; i < dizi.length; i++) {
    ...
    if (fenomen.profile === profil) return fenomen.posts;
  }
}
